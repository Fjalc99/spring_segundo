...
public class ProductoService {
    ...
    public List<Producto> findAll(){
        /*Obtener todos los producto y si esta vacia lanzar excepciones*/
        List<Producto> result  = productoRepository.findAll();
        if (result.isEmpty())
            throw new EntityNotFoundException("No hay productos en la lista");
        return result;
    }
    ...
    public Producto edit (Producto producto, Long id){
        return productoRepository.findById(id).map(old ->{
